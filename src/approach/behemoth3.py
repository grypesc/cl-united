...
class Adapter(torch.nn.Module):
    def __init__(self, adapter_type, S, device):
        super().__init__()
        self.S = S
        self.device = device
        self.nn = nn.Linear(S, S)
        if adapter_type == "mlp":
            self.nn = nn.Sequential(nn.Linear(S, 2 * S),
                                    nn.GELU(),
                                    nn.Linear(2 * S, S)
                                    )
...
class Appr(Inc_Learning_Appr):
    ...
    def train_backbone(self, t, trn_loader, val_loader, num_classes_in_t):
        ...
        for epoch in range(self.nepochs):
            ...
            adapter = Adapter(self.distiller_type, self.S, self.device)
            ...
            with torch.no_grad():
                for images, targets in val_loader:
                    ...
                    adapted_features = distiller(features) if t > 0 else None
                    if t > 0 and epoch > 30:
                        adapted_protos = adapter.adapt_prototypes(self.prototypes)
                        dist = torch.cdist(proxies, adapted_protos)
                        dist = torch.topk(dist ** 2, self.K, 1, largest=False)[0] * self.gamma
                        dist = self.push_fun(dist) / self.N
                        push_loss = -dist.mean()
        ...
        # if t > 0:
        #     with torch.no_grad():
        #         self.prototypes = adapter(trn_loader, val_loader, self.model, self.old_model, self.prototypes, 0.01, self.adapter_epochs)
